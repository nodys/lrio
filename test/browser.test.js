  ...
  describe('browser', function() {
    ...
    function mockXMLHttpRequest(forChannel) {
      return function() {
        ...
        self.open = function() {}
        self.send = function() { self.onreadystatechange() }
      }
    }
    ...
    function watchClient(client) {
      ...
      function onerror()  { result.eventCalled.push('error') }
      function onopen()   { result.eventCalled.push('open') }
      function onclose()  { result.eventCalled.push('close') }
    ...
    it('should listen for server messages on the given channel', function(done) {
      ...
      jsdom.env({
        ...
        'done': function(errors, window) {
          ...
          socket1.onopen()
          socket1.onmessage({data: JSON.stringify({myData: '42'})});
          socket1.onclose()
          socket1.onerror()
          ...
          expect(watch1.message).to.eql({myData: '42'})
          expect(watch1.eventCalled).to.eql(['open','message','close','error'])

          // Socket2 (anotherChannel) must not even be created
          // (the mockXMLHttpRequest accept only myChannel)
          expect(socket2).to.eql(undefined)
